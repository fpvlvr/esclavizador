...
class TestCreateTag:
    ...
    async def test_create_tag_as_master(
        ...
        assert data["organization_id"] == str(test_master["organization_id"])

    async def test_create_tag_as_slave_forbidden(
        self, client, test_slave, test_slave_email, test_slave_password
    ):
        """Test slave cannot create tag (403)."""
        # Login as slave
        login_response = await client.post("/api/v1/auth/login", json={
...
class TestListTags:
    ...
    async def test_list_tags_as_master(
        self, client, test_master, test_master_email, test_master_password
    ):
        """Test master can list tags."""
        # Login
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_master_email,
            "password": test_master_password
    ...
    async def test_list_tags_as_slave(
        self, client, test_slave, test_slave_email, test_slave_password
    ):
        """Test slave can list tags."""
        # Login as slave
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_slave_email,
            "password": test_slave_password
...
class TestGetTag:
    ...
    async def test_get_tag_as_master(
        self, client, test_master, test_master_email, test_master_password
    ):
        """Test master can get tag by ID."""
        # Login
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_master_email,
            "password": test_master_password
    ...
    async def test_get_tag_as_slave(
        self, client, test_slave, test_slave_email, test_slave_password
    ):
        """Test slave can get tag by ID."""
        # Login as slave
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_slave_email,
            "password": test_slave_password
        ...
        # Create tag
        tag = await tag_repo.create("SlaveGetTest", str(test_slave["organization_id"]))
        ...
        await tag_repo.delete(str(tag["id"]), str(test_slave["organization_id"]))
...
class TestUpdateTag:
    ...
    async def test_update_tag_as_master(
        self, client, test_master, test_master_email, test_master_password
    ):
        """Test master can update tag."""
        # Login
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_master_email,
            "password": test_master_password
    ...
    async def test_update_tag_as_slave_forbidden(
        self, client, test_slave, test_slave_email, test_slave_password
    ):
        """Test slave cannot update tag (403)."""
        # Login as slave
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_slave_email,
            "password": test_slave_password
        ...
        # Create tag
        tag = await tag_repo.create("TestTag", str(test_slave["organization_id"]))
...
class TestDeleteTag:
    ...
    async def test_delete_tag_as_master(
        self, client, test_master, test_master_email, test_master_password
    ):
        """Test master can delete tag."""
        # Login
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_master_email,
            "password": test_master_password
    ...
    async def test_delete_tag_as_slave_forbidden(
        self, client, test_slave, test_slave_email, test_slave_password
    ):
        """Test slave cannot delete tag (403)."""
        # Login as slave
        login_response = await client.post("/api/v1/auth/login", json={
            "email": test_slave_email,
            "password": test_slave_password
        ...
        # Create tag
        tag = await tag_repo.create("ToDelete", str(test_slave["organization_id"]))
    ...
    async def test_delete_tag_cascade_removes_from_time_entries(
        ...
        # Create tag
        tag = await tag_repo.create("CascadeTest", str(test_master["organization_id"]))
